...
function initialize_dhtmlx_otu_tree() {
    ...
    otuTree = new dhtmlXTreeObject("otus_select_div","100%","100%",0);
    otuTree.setImagesPath("/images/dhtmlx/imgs/");
    otuTree.enableCheckBoxes(true);
    
    //otuTree.enableTreeLines(true); // true by default
    otuTree.enableTreeImages(false);
    otuTree.attachEvent("onCheck",function(id){
      on_check_dhtmlx(id)
    });
    
    otuTree.attachEvent("onDblClick", function(id){
      expand_tree_dhtmlx(id)
    });
    
    otuTree.attachEvent("onOpenEnd", onNodeSelect);
    otuTree.setXMLAutoLoading("/otus/otu_tree_dhtmlx");
    otuTree.setDataMode("json");

	// MOVED TO CLEAR FILTERS:: loads first level of tree
	otuTree.load("/otus/otu_tree_dhtmlx?id=0","json");
    
}
//
//
//
//     function load_otu_tree(){
// 
...
function set_dataset_count(id){
    ...
    if(Object.keys(datasets_local).length > 0){
      ...
      var checked_nodes = otuTree.getAllChecked().split(',') 
      if(checked_nodes.length == 1 && otuTree.getLevel(checked_nodes[0]) != 2){
        len = 0
      }else{
...
function onNodeSelect(id){
    ...
    var openState = otuTree.getOpenState(id);
    
    if(openState < 1){
        // un-select all on close node
        otuTree.setSubChecked(id, false)
    }else{
        otuTree.setSubChecked(id, true)
        otuTree.setCheck(id,false); 
    }
...
function on_check_dhtmlx(id){
    ...
    var level = otuTree.getLevel(id)
    otuTree.closeAllItems(0)
    if(level === 1){
        // project
        ...
        var openState = otuTree.getOpenState(id);
        if(openState === 1){
            dids = otuTree.getAllSubItems(id);
            didArray = dids.split(',');
            var checkStateZero = otuTree.isItemChecked(didArray[0]);
            if(checkStateZero  === 1){
                //checked
                otuTree.setSubChecked(id, false);
            }else{
...
function clear_filters() {
  ...
  xmlhttp.onreadystatechange=function() {
    if ( xmlhttp.readyState == 4 ) {
        ...
        otuTree.deleteChildItems(0);
        //load initial
        otuTree.load("/visuals/project_dataset_tree_dhtmlx?id=0",afterCall,"json"); 
...
function afterCall(){
    ...
    setTimeout(function() {
        ...
        set_dataset_count(0);
        var cnodes = otuTree.getAllChecked()
        if(cnodes.length > 0){
            var l = cnodes.split(',')
            otuTree.focusItem(otuTree.getParentId(l[0]));
        }
    }, delay);
}
//
