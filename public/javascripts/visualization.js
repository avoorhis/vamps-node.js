...
function load_project_select(datasets_by_project_all){
  //alert('in load')
  var rows = JSON.parse(datasets_by_project_all);
  var html = '<ul>';
  for (var i in rows.projects){
    var pname = rows.projects[i].pname;
    var pid   = rows.projects[i].pid;
    ...
    for (var k in rows.projects[i].datasets) {
      var dname = rows.projects[i].datasets[k].dname;
      var did = rows.projects[i].datasets[k].did;
      var pd = pname+'--'+dname;
      html += "<li>";
