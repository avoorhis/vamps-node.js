...
#             sys.exit("3-ConfigFile conversion to dictionary not written yet for platform ("+self.args.platform+") ")
        ...
        print "self.new_vamps_upload",self.new_vamps_upload
        if not self.vamps_user_upload and not self.new_vamps_upload:
    ...
    def initializeFromDictionary(self, configDict):
        ...
        logging.info( 'General Config:',general_config)
        ...
        if self.vamps_user_upload or self.new_vamps_upload:
            logging.info( 'getting vamps stuff in runconfig')
            self.user           = general_config['user']           
            #logging.info(general_config['user'] ) 
