...
def go_distance(args):
    ...
    dm1 = get_dist(args.metric, dmatrix)
    # [  9.91362925e-01   3.12299075e-02   2.78586503e-02   9.95047186e-01   9.94946860e-01   1.03170419e-04]
    dist = dm1.condensed_form()  ## see http://scikit-bio.org/docs/0.2.2/generated/generated/skbio.stats.distance.DistanceMatrix.html?highlight=distancematrix
    #dm1 = get_data_matrix1(dist)
    #print('dist1')
    #print(dm1)
    #dist1 = distance.squareform( dist )
    #print('dist')
    #print(dist)
    ...
    for row,name in enumerate(edited_dataset_list):
            ...
            for col,d in enumerate(dm1[row]):
                ...
                jsondm2[name][data['columns'][col]['id']]  = dm1[row][col]
                jsondm3[(name, (data['columns'][col]['id']))]  = dm1[row][col]
    ...
    return (dm1, dist, jsondm2, jsondm3, edited_dataset_list)
# dm1: [[]]
#[
...
if __name__ == '__main__':
    ...
    For Anaconda and python 3 change the PATH variable in config.js
    """
    ...
    ( dm1, dist, jsondm2, jsondm3, datasets ) = go_distance(args)

    if args.function == 'cluster_datasets':
        #did_list = cluster_datasets(args, dm3, did_hash)
        new_ds_list = cluster_datasets(args, jsondm3)
    ...
    if args.function == 'dheatmap':
        ...
        print (json.dumps(jsondm2))

    if args.function == 'dendrogram-svg':

        newick = dendrogram_svg(args, jsondm3)
    ...
    if args.function == 'pcoa_2d':
        ...
        #pcoa_data = get_pcoa(args, jsondm3)
        pcoa_data = get_pcoa(args, dm1)
