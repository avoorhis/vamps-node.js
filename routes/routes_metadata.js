...
router.post('/metadata_new',
  ...
  form(
    ...
    form.field("d_region").trim().required("", "Domain and region are required").entityEncode(),
    ...
    form.field("pi_id_name").trim().required("", "PI name is required").entityEncode(),
    form.field("project_description").trim().required("", "Project description is required").entityEncode(),
    form.field("project_name1").trim().entityEncode(),
    form.field("project_name2").trim().required("", "Project name is required").entityEncode(),
    form.field("project_title").trim().required("", "Project title is required").is(/^[a-zA-Z0-9,_ -]+$/).entityEncode(),
    form.field("reference").trim().entityEncode(),
